	...
	cv::Mat &output, cv::Point2i location)
{
	...
	int nChns_fg = foreground.channels();
	int nChns_op = output.channels();
	int nStep_fg = foreground.step;
	int nStep_op = output.step;
	...
	for (int y = std::max(location.y, 0); y < output.rows; ++y)
	{
		...
		for (int x = std::max(location.x, 0); x < output.cols; ++x)
		{
			...
			// determine the opacity of the foregrond pixel, using its fourth (alpha) channel.
			double opacity =
				((double)foreground.data[fY * nStep_fg + fX * nChns_fg + 3])
			...
			// but only if opacity > 0.
			for (int c = 0; opacity > 0 && c < nChns_op; ++c)
			{
				unsigned char foregroundPx =
					foreground.data[fY * nStep_fg + fX * nChns_fg + c];
				//unsigned char backgroundPx =
				//	background.data[y * nStep_bg + x * nChns_bg + c];
				unsigned char o_data = output.data[y*nStep_op + nChns_op*x + c];
				output.data[y*nStep_op + nChns_op*x + c] =
					o_data * (1. - opacity) + foregroundPx * opacity;
			}
		}
	}
}
