...
void CIqa::stat_nb_diff_h(vector<NBDiffInfo> &vInfo, Mat &g, Mat &nb_diff_1p, Mat &nb_diff_2p)
{
	...
	{
		NBDiffInfo info;
		Mat tmpImg = nb_diff_1p(Rect(0, i, nb_diff_1p.cols, 1));
		int zero_count = nb_diff_1p.cols - calcNotZeroCount(tmpImg);
		info._abssum = sum(cv::abs(nb_diff_2p(Rect(0, i, nb_diff_2p.cols, 1))))[0];
		int col_point_count = (nb_diff_2p.cols + 0 - zero_count);
		if (col_point_count <= 0)
			col_point_count = 1;
		info._absmean = info._abssum / col_point_count;
...
void CIqa::stat_nb_diff_v(vector<NBDiffInfo> &vInfo, Mat &g, Mat &nb_diff_1p, Mat &nb_diff_2p)
{
	...
	{
		NBDiffInfo info;
		Mat tmpImg = nb_diff_1p(Rect(i, 0, 1, nb_diff_1p.rows));
		int zero_count = nb_diff_1p.rows - calcNotZeroCount(tmpImg);
		info._abssum = sum(cv::abs(nb_diff_2p(Rect(i, 0, 1, nb_diff_2p.rows))))[0];
		int row_point_count = (nb_diff_2p.rows + 0 - zero_count);
		if (row_point_count <= 0)
			row_point_count = 1;
		info._absmean = info._abssum / row_point_count;
...
float CIqa::calcGrayGI(Mat &g)
{
	...
	//cout << info_b._gi_h << " " << info._gi_h << endl;
	if (info._gi_v == 0)
		score_v = 1;
	else
		score_v = info_b._gi_v / info._gi_v;
	//cout << info_b._gi_v << " " << info._gi_v << endl;
...
float CIqa::calcMos(Mat &img, int bHaveBI)
{
	...
	float score_gi, score_gi_d8, score_bi = 1.0;
...
float CIqa::calcContentFactor(Mat &img)
{
	int i, j;

	// 先算能量
	Mat img_4x4 = img(Rect(0, 0, (img.cols / 4) * 4, (img.rows / 4) * 4));
	
	double fEnergy = 0;
	int count_low = 0, count_high = 0;
	for (i = 0; i < img.rows / 4; i++)
	{
		for (j = 0; j < img.cols / 4; j++)
		{
			Mat block_4x4_g;
			Mat block_4x4 = img(Rect(j * 4, i * 4, 4, 4));
			cvtColor(block_4x4, block_4x4_g, CV_BGR2GRAY);
			float sad = mat_mean_sad(block_4x4_g);
			if (sad < 4)
				count_low++;
			if (sad > 80 )
				count_high++;
	...
	int total = (img_4x4.cols*img_4x4.rows/16);
	if (total == 0)
		total = 1;

//#if TOUTIAOTUIJIAN
#if 0
	float fMeanEnergy = fEnergy / (total * 16);
	float fContentBlockPer = 1 - float(count_low) / total;
	return (1 - fMeanEnergy / 192) * 7 / float(7 + fContentBlockPer);
#else
	float factor = 1;
	float hiblock_ratio = float(count_high) / total;
	if (hiblock_ratio < 0.10)
	{
		factor = 0.95 + (0.10 - hiblock_ratio) / 2;
	}
	else
	{
		if (hiblock_ratio<0.20)
			factor = 0.95 - (hiblock_ratio - 0.10);
		else
			factor = 0.95 - (hiblock_ratio-0.10)*1.2;
		if (factor < 0.80)
			factor = 0.80;
	}
	factor = 1 - hiblock_ratio/2;
	return factor;
#endif
}

float CIqa::calcContrastFactor(Mat &img)
{
	float factor = 1.0;
	int grayHist[64];
	g_hist1D.mat2GrayHist(img, grayHist);
	...
	for (int i = 0; i < 64; i++)
	{
		...
		if (low_id < 0)
		{
			if (sum > img.cols*img.rows / 8)
				low_id = i;
		}
		if (high_id < 0)
		{
			if (sum > img.cols*img.rows*7 / 8)
				high_id = i;
		}
	}
	//cout << low_id << "  " << high_id << endl;
	factor = float(high_id - low_id) / 32;
	factor = pow(factor, 0.33);
	//cout << factor << endl;

	return factor;
}

float CIqa::calcColorFactor(Mat &img)
{
	float factor = 1.0;
	Mat img256;
	resize(img, img256, Size(256, 256));
	
	Mat rgbImg[3], difImg0, difImg1, maxImg;
	split(img256, rgbImg);
	absdiff(rgbImg[0], rgbImg[1], difImg0);
	absdiff(rgbImg[0], rgbImg[2], difImg1);
	cv::max(difImg0, difImg1, maxImg);
	imshow("11", maxImg);
	waitKey();
	int hist_array[64];
	g_hist1D.mat2GrayHist(maxImg, hist_array);
	for (int i = 0; i < 64; i++)
		cout << hist_array[i] << " ";
	cout << endl;
...
float CIqa::calcBlockingFactor(Mat &img)
{
	float factor = 1.0;
	Mat lineDiff_v, lineDiff_h;
	Mat img_g;
	cvtColor(img, img_g, CV_BGR2GRAY);
	//resize(img_g, img_g, Size(img_g.cols/2, img_g.rows/2));

	int w = img_g.cols, h = img_g.rows;

	absdiff(img_g(Rect(0, 0, w, h - 1)), img_g(Rect(0, 1, w, h -1)), lineDiff_v);
	absdiff(img_g(Rect(0, 0, w -1, h)), img_g(Rect(1, 0, w - 1, h)), lineDiff_h);
	int count_v = 0;
	const int stride = 8;
	int en = 7;
	for (int i = 1; i < (lineDiff_v.rows / stride - 1); i++)
	{
		for (int j = 0; j < lineDiff_v.cols; j++)
		{
			uchar v[stride], v_max;
			for (int k = 0; k < stride; k++)
				v[k] = lineDiff_v.ptr<uchar>(i * stride + k)[j];
			v_max = max_v4(v, stride);
			if ((lineDiff_v.ptr<uchar>(i * stride + en)[j] > lineDiff_v.ptr<uchar>(i * stride + en - 1)[j]) && \
				(lineDiff_v.ptr<uchar>(i * stride + en)[j] > lineDiff_v.ptr<uchar>(i * stride + en + 1)[j]))
			{
				lineDiff_v.ptr<uchar>(i * stride + en)[j] = 255;
				count_v++;
			} else
				lineDiff_v.ptr<uchar>(i * stride + en)[j] = 0;
				
			for (int k = 0; k < stride; k++)
			{
				if (lineDiff_v.ptr<uchar>(i * stride + k)[j] != 255)
					lineDiff_v.ptr<uchar>(i * stride + k)[j] = 0;
			}
		}
	}
	cout << "count_v : " << float(count_v) / (lineDiff_v.cols*lineDiff_v.rows) << endl;

	imshow("11", lineDiff_v);
	imshow("12", lineDiff_h*10);
	waitKey();
