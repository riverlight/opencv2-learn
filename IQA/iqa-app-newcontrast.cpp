...
#else
		img = a(Rect(a.cols / 16, a.rows / 16, a.cols - a.cols / 8, a.rows - a.rows / 8));
#endif
	}
	else
		img = a;

	float fContentFactor = g_iqa.calcContentFactor(img);
#if TOUTIAOTUIJIAN
	float fContrastFactor = 1.0;
#else
	float fContrastFactor = g_iqa.calcContrastFactor(img);
#endif
	cout << fContentFactor << "  " << fContrastFactor <<endl;
