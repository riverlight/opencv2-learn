...
static float g_fCosSimi_frames_threshold = 0.99;
static float g_fCosSimi_start_end_threshold = 0.92;
...
void fis2sis(vector<FrameInfo> &fis, vector<SceneInfo> &sis_out)
{
	...
	for (int i = 0; i < fis.size(); i++)
	{
		...
		if (fCosSimi_frames < g_fCosSimi_frames_threshold || fCosSimi_start_end<g_fCosSimi_start_end_threshold)
		{
...
int output_sis(vector<SceneInfo> &sis, vector<FrameInfo> &fis, char *szVFile, char *szCsvfile, char *szWorkpath)
{
	...
	for (int i = 0; i < sis.size(); i++)
	{
		...
		if (sis[i]._endtime_ms - sis[i]._starttime_ms < 150)
			continue;
...
int main(int argc, char *argv[])
{
	...
	if (argc == 6)
	{
		g_fCosSimi_frames_threshold = atof(argv[4]);
		g_fCosSimi_start_end_threshold = atof(argv[5]);
	}
